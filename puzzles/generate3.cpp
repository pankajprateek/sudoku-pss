...
bool SudokuBoard::colBoxReduction(int round){
    for (int valIndex=0; valIndex<NUM_POSS; valIndex++){
        for (int col=0; col<GRID_SIZE*GRID_SIZE; col++){
...
bool SudokuBoard::pointingRowReduction(int round){
    for (int valIndex=0; valIndex<NUM_POSS; valIndex++){
        for (int section=0; section<GRID_SIZE*GRID_SIZE; section++){
            int secStart = sectionToFirstCell(section);
            bool inOneRow = true;
            int boxRow = -1;
            for (int j=0; j<GRID_SIZE; j++){
                {for (int i=0; i<GRID_SIZE; i++){
                    int secVal=secStart+i+(GRID_SIZE*GRID_SIZE*j);
    ...
    for (int valIndex=0; valIndex<NUM_POSS; valIndex++){
        for (int section=0; section<GRID_SIZE*GRID_SIZE; section++){
            ...
            if (inOneCol && boxCol != -1){
                ...
                {for (int i=0; i<GRID_SIZE*GRID_SIZE; i++){
                    int position = colStart+(GRID_SIZE*GRID_SIZE*i);
                    int section2 = cellToSection(position);
                    int valPos = getPossibilityIndex(valIndex,position);
                    if (section != section2 && possibilities[valPos] == 0){
                        possibilities[valPos] = round;
...
bool SudokuBoard::hiddenPairInColumn(int round){
    for (int column=0; column<GRID_SIZE*GRID_SIZE; column++){
        for (int valIndex=0; valIndex<GRID_SIZE*GRID_SIZE; valIndex++){
            int r1 = -1;
            int r2 = -1;
            int valCount = 0;
            for (int row=0; row<GRID_SIZE*GRID_SIZE; row++){
            ...
            if (valCount==2){
                for (int valIndex2=valIndex+1; valIndex2<GRID_SIZE*GRID_SIZE; valIndex2++){
                    int r3 = -1;
                    int r4 = -1;
                    int valCount2 = 0;
                    for (int row=0; row<GRID_SIZE*GRID_SIZE; row++){
                    ...
                    if (valCount2==2 && r1==r3 && r2==r4){
                        bool doneSomething = false;
                        for (int valIndex3=0; valIndex3<GRID_SIZE*GRID_SIZE; valIndex3++){
...
bool SudokuBoard::hiddenPairInSection(int round){
    for (int section=0; section<GRID_SIZE*GRID_SIZE; section++){
        for (int valIndex=0; valIndex<GRID_SIZE*GRID_SIZE; valIndex++){
            ...
            int si2 = -1;
            int valCount = 0;
            for (int secInd=0; secInd<GRID_SIZE*GRID_SIZE; secInd++){
                ...
                if (position != position2){
                    ...
                    if (possibilities2 == 2 && arePossibilitiesSame(position, position2)){
                        if (row == cellToRow(position2)){
                            bool doneSomething = false;
                            for (int column2=0; column2<GRID_SIZE*GRID_SIZE; column2++){
                        ...
                        if (column == cellToColumn(position2)){
                            bool doneSomething = false;
                            for (int row2=0; row2<GRID_SIZE*GRID_SIZE; row2++){
                        ...
                        if (section == cellToSectionStartCell(position2)){
                            ...
                            {for (int i=0; i<GRID_SIZE; i++){
                                for (int j=0; j<GRID_SIZE; j++){
                                    int position3=secStart+i+(GRID_SIZE*GRID_SIZE*j);
                                    if (position3 != position && position3 != position2 && removePossibilitiesInOneFromTwo(position, position3, round)){
                                        doneSomething = true;
                                    }
                                }
...
bool SudokuBoard::onlyValueInSection(int round){
    for (int sec=0; sec<SEC_COUNT; sec++){
        ...
        for (int valIndex=0; valIndex<NUM_POSS; valIndex++){
            ...
            {for (int i=0; i<GRID_SIZE; i++){
                for (int j=0; j<GRID_SIZE; j++){
                    int position = secPos + i + GRID_SIZE*GRID_SIZE*j;
                    int valPos = getPossibilityIndex(valIndex,position);
                    if (possibilities[valPos] == 0){
                        count++;
                        lastPosition = position;
...
void SudokuBoard::mark(int position, int round, int value){
    ...
    int rowStart = cellToRow(position)*GRID_SIZE*GRID_SIZE;
    for (int col=0; col<COL_HEIGHT; col++){
    ...
    {for (int i=0; i<GRID_SIZE*GRID_SIZE; i++){
        int colVal=colStart+(GRID_SIZE*GRID_SIZE*i);
...
void SudokuBoard::printPossibilities(){
    for(int i=0; i<BOARD_SIZE; i++){
        cout << " ";
        for (int valIndex=0; valIndex<NUM_POSS; valIndex++){
            int posVal = (GRID_SIZE*GRID_SIZE*i)+valIndex;
            int value = valIndex+1;
        ...
        if (i != BOARD_SIZE-1 && i%SEC_GROUP_SIZE==SEC_GROUP_SIZE-1){
            cout << endl << "-------------------------------|-------------------------------|-------------------------------" << endl;
        } else if (i%(GRID_SIZE*GRID_SIZE)==GRID_SIZE*GRID_SIZE-1){
            cout << endl;
 ...
 */
static inline int rowToFirstCell(int row){
    return GRID_SIZE*GRID_SIZE*row;
}
